...
export default function RepositoryDashboard() {
  ...
  const renderTreeNode = (node: any, level = 0) => {
    ...
    if (!node.isDirectory) {
      ...
      return (
        <div
          key={node.id}
          className={`flex justify-between items-center group py-2 px-3 cursor-default transition-all duration-300 border-b border-lux-border/30 last:border-0 text-xs pl-[calc(var(--indent-level)*0.4rem+0.4rem)] md:pl-[calc(var(--indent-level)*1.2rem+0.75rem)] ${isSelected ? 'bg-lux-gold/10' : 'hover:bg-lux-card/40'}`}
        ...
        >
          <div className="flex items-center gap-2 truncate pr-4">
            ...
            <span className={`truncate text-xs font-mono ${isSelected ? 'text-lux-gold font-bold' : 'text-lux-creme'}`} title={node.name}>
              {node.name}
            </span>
          </div>
          <div className="text-[10px] text-lux-creme-dim flex-shrink-0 flex items-center gap-3">
            ...
            <button
              ...
              className={`px-2 py-0.5 border text-[9px] uppercase tracking-wider transition-colors font-semibold ${isSelected
    ...
    return (
      <div key={node.path}>
        <div
          ...
          className="flex items-center gap-2 hover:bg-lux-card/30 py-2 px-3 cursor-pointer border-b border-lux-border/30 last:border-0 text-xs pl-[calc(var(--indent-level)*0.4rem+0.4rem)] md:pl-[calc(var(--indent-level)*1.2rem+0.75rem)]"
          style={{ '--indent-level': level } as React.CSSProperties}
        >
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-12rem)] w-full max-w-xl mx-auto p-8 space-y-6 animate-reveal-up text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[10px] tracking-wider text-lux-creme-dim uppercase">
            Deconstructing files into vector embeddings
          </p>
        </div>
        <div className="w-full border border-lux-border p-4 bg-lux-card/10 space-y-2 text-xs text-lux-creme-dim">
        ...
        <p className="text-[10px] text-lux-gold animate-pulse tracking-widest uppercase">
  ...
  const isModalOpen = isChatFullscreen || isExpandedView || isInsightOpen;

  return (
    <div className={`flex flex-col h-[calc(100vh-8rem)] w-full max-w-[1800px] mx-auto -mt-2 md:-mt-6 space-y-4 ${isModalOpen ? '' : 'animate-reveal-up'}`}>

      {/* Header Panel */}
      <div className="border border-lux-border py-2.5 px-4 md:py-3 md:px-5 bg-lux-card/15 backdrop-blur-md flex flex-col md:flex-row justify-between items-start md:items-center shadow-lux gap-4">
        <div className="flex items-center gap-3">
          <Link
            href="/dashboard"
            className="p-1.5 border border-lux-border text-lux-creme-dim hover:text-lux-gold hover:border-lux-gold transition-colors duration-300 flex items-center justify-center mt-0.5"
            title="Return to Repository Index"
          >
            <ArrowLeft className="w-3.5 h-3.5" />
          </Link>
          <div>
            <h1 className="text-lg md:text-xl font-serif font-light tracking-widest text-lux-creme flex items-center gap-2.5 uppercase">
              <HardDrive className="w-5 h-5 text-lux-gold" />
              {repo.name}
              <button
                onClick={handleOpenInsights}
                className="p-1 text-lux-gold hover:text-lux-creme hover:bg-lux-card border border-lux-border hover:border-lux-gold transition-all duration-300 flex items-center justify-center ml-1"
                title="Show System Insights"
              >
                <Info className="w-3.5 h-3.5" />
              </button>
            </h1>
            <p className="text-[9px] tracking-widest text-lux-creme-dim mt-1.5 flex flex-wrap items-center gap-2.5 uppercase">
        ...
        <div className="flex items-center gap-3 border-t pt-3 mt-1.5 w-full md:w-auto md:border-t-0 md:pt-0 md:mt-0 md:border-l md:pl-5 border-lux-border">
          <Cpu className="w-8 h-8 text-lux-gold opacity-60" />
          <div className="text-[9px] text-lux-creme-dim text-left uppercase tracking-wider">
      ...
      {/* Mobile Tab Control */}
      <div className="flex md:hidden border border-lux-border text-xs bg-lux-card/25">
        <button
          onClick={() => setActiveMobileTab('chat')}
          className={`flex-1 py-3 text-center uppercase tracking-widest font-bold transition-all ${activeMobileTab === 'chat'
        ...
        <button
          ...
          className={`flex-1 py-3 text-center uppercase tracking-widest font-bold transition-all ${activeMobileTab === 'files'
              ? 'text-lux-gold bg-lux-card/40 border-b border-lux-gold'
              : 'text-lux-creme-dim hover:text-lux-creme'
            }`}
        >
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0">
        ...
        <div className={`md:w-1/3 border border-lux-border flex flex-col h-full bg-lux-card/15 backdrop-blur-md shadow-lux ${isChatFullscreen ? 'hidden' : 'flex'
          ...
          <div className="border-b border-lux-border p-4 bg-lux-card/45 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h2 className="uppercase tracking-widest text-xs font-bold text-lux-creme">{c.explorerTreeTitle}</h2>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-0.5 scrollbar-thin">
            {files.length === 0 ? (
              <p className="text-[10px] text-lux-creme-dim italic p-4">{c.allFilesOffline}</p>
            ) : (
              files.map(file => (
                <div key={file.id} className="flex justify-between items-center group hover:bg-lux-card/30 p-2 cursor-default transition-all duration-300 border-b border-lux-border/30 last:border-0 text-xs">
                  <div className="flex items-center gap-2 truncate pr-4">
                  ...
                  <div className="text-[9px] text-lux-creme-dim flex-shrink-0 flex gap-3 uppercase tracking-wider items-center">
        ...
        <div className={`flex flex-col overflow-hidden ${isChatFullscreen
            ? 'fixed inset-0 z-[100] p-6 bg-lux-bg/95 backdrop-blur-md'
          ...
          <div className={`flex-1 flex flex-col min-h-0 bg-lux-card/10 relative ${isChatFullscreen
            ...
            <div className="border-b border-lux-border p-4 bg-lux-card/45 flex items-center justify-between z-20">
              <div className="flex items-center gap-2">
                ...
                <h2 className="uppercase tracking-widest text-xs font-bold text-lux-creme">{c.chatHeaderTitle}</h2>
              </div>
            ...
            {/* Chat Messages */}
            <div className="flex-1 overflow-y-auto p-6 space-y-6 flex flex-col text-xs scrollbar-thin">
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex flex-col min-w-0 ${isChatFullscreen ? 'w-full max-w-full' : 'w-full max-w-[90%] md:max-w-[80%]'} ${msg.role === 'user' ? 'self-end items-end' : 'self-start'}`}>
                  ...
                  <div
                    className={`p-4 border text-xs leading-relaxed w-full min-w-0 break-words ${msg.role === 'user'
                  ...
                  >
                    ...
                    {msg.sources && msg.sources.length > 0 && (
                      <div className="mt-4 pt-3 border-t border-lux-border/50 text-[10px]">
                        <span className="text-lux-gold font-bold uppercase tracking-wider block mb-2 text-[9px]">{c.citationsLabel}</span>
            ...
            <div className="border-t border-lux-border p-4 bg-lux-card/25 backdrop-blur-md z-20">
              {repo?.status === 'error' || repo?.chunkCount === 0 ? (
                <div className="flex items-center justify-center gap-2 py-2 text-lux-copper text-xs border border-dashed border-lux-copper/35 bg-lux-copper/5 rounded">
                ...
                <form onSubmit={handleSendMessage} className="flex gap-4 items-center">
                  <span className="text-sm text-lux-gold font-bold animate-pulse">&gt;</span>
                  <input
                    type="text"
                    value={chatInput}
                    ...
                    disabled={isSending}
                    className="flex-1 bg-transparent border-b border-lux-border/60 focus:border-lux-gold/50 text-lux-creme placeholder:text-lux-creme-dim/30 px-2 py-2 outline-none text-xs"
                  />
                  <button
                    type="submit"
      ...
      {isExpandedView && (
        <div className="fixed inset-0 z-[100] bg-lux-bg/95 backdrop-blur-md p-4 md:p-6 flex flex-col">
          {/* Modal Header */}
          ...
          <div className="flex md:hidden border border-lux-border mb-4 text-xs bg-lux-card/25">
            <button
              onClick={() => setActiveWorkspaceTab('files')}
              className={`flex-1 py-3 text-center uppercase tracking-widest font-bold transition-all ${activeWorkspaceTab === 'files'
                  ? 'text-lux-gold bg-lux-card/40 border-b border-lux-gold'
                  : 'text-lux-creme-dim hover:text-lux-creme'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-3 text-center uppercase tracking-widest font-bold transition-all ${activeWorkspaceTab === 'code'
                  ? 'text-lux-gold bg-lux-card/40 border-b border-lux-gold'
                  : 'text-lux-creme-dim hover:text-lux-creme'
                }`}
            >
          ...
          <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0 animate-reveal-up delay-100">
            ...
            <div className={`flex-1 md:flex-[0.3] border border-lux-border flex flex-col bg-lux-card/15 overflow-hidden ${activeWorkspaceTab === 'files' ? 'flex' : 'hidden md:flex'
              }`}>
              <div className="border-b border-lux-border p-4 bg-lux-card/45 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Folder className="w-4 h-4 text-lux-gold" />
                  <span className="text-[10px] tracking-widest text-lux-creme font-bold uppercase">{c.workspaceFileHierarchy}</span>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="px-2 py-0.5 border border-lux-border text-[9px] text-lux-creme-dim hover:text-lux-gold hover:border-lux-gold uppercase transition-colors"
                >
            ...
            <div className={`flex-1 md:flex-[0.7] border border-lux-border flex flex-col bg-lux-card/15 overflow-hidden ${activeWorkspaceTab === 'code' ? 'flex' : 'hidden md:flex'
              }`}>
              <div className="border-b border-lux-border p-4 bg-lux-card/45 flex justify-between items-center">
                <div className="flex items-center gap-2 truncate pr-4">
                ...
                {selectedFile && (
                  <div className="text-[9px] text-lux-creme-dim flex-shrink-0 flex gap-4 uppercase tracking-wider">
              ...
              <div className="flex-1 bg-[#12100f] relative overflow-hidden">
                {isFileLoading ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-lux-bg/90 z-20 space-y-3">
                    ...
                    <p className="text-lux-gold animate-pulse text-[10px] uppercase tracking-widest">&gt; {c.workspaceStreamingFile}</p>
                  </div>
                ) : selectedFile ? (
                    ...
                    <Editor
                      ...
                      loading={
                        <div className="flex flex-col items-center justify-center h-full bg-[#12100f] space-y-3">
                          <p className="text-lux-gold animate-pulse text-[10px] uppercase tracking-widest">&gt; {c.workspaceLaunchingEditor}</p>
                        </div>
                      }
                    />
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-lux-creme-dim space-y-4 p-8 text-center select-none">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] opacity-60 uppercase tracking-wider">{c.workspaceAwaitTargetDesc}</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* Insight Panel Modal */}
      {isInsightOpen && (
        <div className="fixed inset-0 z-[100] bg-lux-bg/95 backdrop-blur-md p-6 flex flex-col animate-fade-in">
          {/* Modal Header */}
          ...
          <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0 overflow-y-auto animate-reveal-up delay-100">
            ...
            <div className="flex-1 md:flex-[0.65] border border-lux-border flex flex-col bg-lux-card/15 overflow-hidden">
              <div className="border-b border-lux-border p-4 bg-lux-card/45">
                <span className="text-[10px] tracking-widest text-lux-gold uppercase font-bold">{copyContent.dashboard.insightsAiBlueprint}</span>
              </div>

              <div className="flex-1 overflow-y-auto p-6 space-y-4 text-lux-creme-dim text-xs select-text">
                {isSummaryLoading ? (
            ...
            <div className="flex-1 md:flex-[0.35] border border-lux-border flex flex-col bg-lux-card/15 overflow-hidden">
              <div className="border-b border-lux-border p-4 bg-lux-card/45">
                <span className="text-[10px] tracking-widest text-lux-gold uppercase font-bold">{copyContent.dashboard.insightsMetrics}</span>
              </div>

              <div className="flex-1 overflow-y-auto p-6 space-y-6 text-lux-creme text-xs scrollbar-thin">
                <StatsSummary
